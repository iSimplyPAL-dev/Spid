...
namespace Italia.Spid.AspNet.WebApp.Controllers
{
    public class HomeController : Controller
    {
       ...
       
        {
            ...
            IList<IDP_Name> IDPList = new List<IDP_Name>() {
                    ...
                    new IDP_Name(){ Position=8, IDPName="https://login.id.tim.it/affwebservices/public/saml2sso", URL_Image = "~/Content/images/spid-idp-titrust.png", Title="TI Trust Technologies"  },
    ...
    new IDP_Name(){ Position=0, IDPName="", URL_Image = "" , Title="Vuoto" },
            ...
            for (int i = 1; i < 11; i++)
            {
                if (i==1)
                     {
                    numeroCasuale = random.Next(1, 10);
                }
                else
                {
                    do {                            
                ...
                while (IDPListRandom[i-1].Position== IDPListRandom[i-2].Position);
           
                }
                IDPListRandom[i - 1] = IDPList[numeroCasuale - 1];
            }

                ViewBag.IDPLIst = IDPListRandom.OrderBy(IDP_Name => IDP_Name.Position);
